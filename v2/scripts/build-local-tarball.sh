...
# Copy built library
echo "${BLUE}   → Copying built library...${NC}"
...
echo "${BLUE}   → Copying components (excluding v1, specifications, specification)...${NC}"
...
# Copy design tokens
echo "${BLUE}   → Copying design tokens...${NC}"
...
echo "${BLUE}   → Creating metadata...${NC}"
...
echo "${BLUE}   → Building component registry...${NC}"
...
# Success!
echo ""
echo "${GREEN}✅ Tarball created successfully!${NC}"
echo ""
...
echo "   ${BLUE}tar -tzf $DIST_DIR/$TARBALL_NAME | head -20${NC}"
echo ""
