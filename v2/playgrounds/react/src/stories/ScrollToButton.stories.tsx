...
import type { ScrollToButtonProps } from "agnosticui-core/scroll-to-button/react";

const meta: Meta<ScrollToButtonProps> = {
  title: "AgnosticUI React/ScrollToButton",
...
export const BackToTop: Story = {
  render: (args: ScrollToButtonProps) => (
    <div style={{ minHeight: "200vh", padding: "20px" }}>
      <h1>Scroll to Top (Default Use Case)</h1>
      <p>
        The button will appear after scrolling {args.scrollThreshold}px down the
        page.
      </p>
      ...
      <div
        style={{
          padding: "20px",
          background: "var(--ag-primary-background, #f0f9ff)",
          borderRadius: "8px",
          margin: "20px 0",
        }}
      >
        <strong>Try it:</strong> Scroll down this long page, then click the
        floating button to return to the top.
      </div>
      {Array.from({ length: 30 }).map((_, i) => (
        <p key={i} style={{ marginBottom: "40px", lineHeight: 1.6 }}>
          Paragraph {i + 1}: Lorem ipsum dolor sit amet, consectetur adipiscing
          elit. Sed do eiusmod tempor incididunt ut labore et dolore magna
          aliqua.
        </p>
      ))}
      <ReactScrollToButton {...args} />
    </div>
  ),
  args: {
    label: "Back to Top",
...
export const ScrollToBottom: Story = {
  render: (args: ScrollToButtonProps) => (
    <div style={{ minHeight: "200vh", padding: "20px" }}>
      ...
      <p>
        This button scrolls to the <strong>bottom</strong> of the page instead
        of the top.
      </p>
      <p>
        Useful for chat interfaces, comment sections, or "jump to end"
        functionality.
      </p>
      <div
        style={{
          padding: "20px",
          background: "var(--ag-success-background, #f0fdf4)",
          borderRadius: "8px",
          margin: "20px 0",
        }}
      >
        <strong>Try it:</strong> Scroll down a bit, then click the button to
        jump to the very bottom.
      </div>
      {Array.from({ length: 30 }).map((_, i) => (
        <p key={i} style={{ marginBottom: "40px", lineHeight: 1.6 }}>
          Content section {i + 1}: This is placeholder content to make the page
          scrollable. In a real application, this might be chat messages,
          comments, or article content.
        </p>
      ))}
      <div
        id="bottom-marker"
        style={{
          padding: "30px",
          background: "var(--ag-success-background, #f0fdf4)",
          border: "2px solid var(--ag-success, #22c55e)",
          borderRadius: "8px",
          textAlign: "center",
        }}
      >
...
export const JumpToSection: Story = {
  render: (args: ScrollToButtonProps) => (
    <div style={{ minHeight: "200vh", padding: "20px" }}>
      ...
      <p>
        Click the button to jump directly to <strong>Section 3</strong>.
      </p>
      <p>
        Useful for table of contents, "skip to main content", or section
        navigation.
      </p>
      ...
      <nav
        style={{
          padding: "20px",
          background: "var(--ag-info-background, #f3f4f6)",
          borderRadius: "8px",
          margin: "20px 0",
        }}
      >
        ...
        <ul style={{ margin: "10px 0", paddingLeft: "20px" }}>
          ...
          <li>
            <strong>Section 3 (button target)</strong>
          </li>
      ...
      <section
        ...
        style={{
          ...
          padding: "30px",
          background: "white",
          border: "2px solid var(--ag-border, #e5e7eb)",
          borderRadius: "8px",
        }}
      >
        <h2>📄 Section 1</h2>
      ...
      <section
        ...
        style={{
          margin: "60px 0",
          padding: "30px",
          background: "var(--ag-warning-background, #fef3c7)",
          border: "3px solid var(--ag-warning, #f59e0b)",
          borderRadius: "8px",
        }}
      >
        <h2>🎯 Section 3 - Target Section!</h2>
        <p>
          <strong>The button scrolls here!</strong> This demonstrates jumping to
          a specific page section.
        </p>
        {Array.from({ length: 5 }).map((_, i) => (
          <p key={i}>This is the target section content...</p>
        ))}
      </section>
...
export const CustomIcon: Story = {
  render: (args: ScrollToButtonProps) => (
    <div style={{ minHeight: "200vh", padding: "20px" }}>
      ...
      <p>
        This example shows how to provide your own custom icon via slot
        projection.
      </p>
...
export const CustomIconEmoji: Story = {
  render: (args: ScrollToButtonProps) => (
    <div style={{ minHeight: "200vh", padding: "20px" }}>
      ...
      <ReactScrollToButton {...args}>
        <span slot="icon" style={{ fontSize: "20px" }}>
          🚀
        </span>
      </ReactScrollToButton>
    </div>
...
export const NoIcon: Story = {
  render: (args: ScrollToButtonProps) => (
    <div style={{ minHeight: "200vh", padding: "20px" }}>
      ...
      <p>
        The button can be rendered without any icon by setting{" "}
        <code>icon={"{false}"}</code>. In this case, the label will be rendered
        regardless to prevent an empty box from being displayed.
      </p>
      {Array.from({ length: 25 }).map((_, i) => (
        <p key={i} style={{ marginBottom: "40px", lineHeight: 1.6 }}>
...
export const WithLabel: Story = {
  render: (args: ScrollToButtonProps) => (
    <div style={{ minHeight: "200vh", padding: "20px" }}>
      ...
      <p>
        The button can show both an icon and label text by setting{" "}
        <code>showLabel={"{true}"}</code>.
      </p>
      <p>
        This creates a more descriptive button that's helpful for users who
        prefer explicit text labels.
      </p>
      <div
        style={{
          padding: "20px",
      ...
      >
        <strong>Try it:</strong> Scroll down to see the button with both icon
        and text label.
      </div>
      {Array.from({ length: 25 }).map((_, i) => (
        <p key={i} style={{ marginBottom: "40px", lineHeight: 1.6 }}>
...
export const CSSPartsCustomization: Story = {
  render: (args: ScrollToButtonProps) => (
    <>
      ...
      `}</style>
      <div style={{ minHeight: "300vh", padding: "20px" }}>
        <h1>CSS Shadow Parts Customization</h1>
        <p>
          Customize the button using CSS Shadow Parts without breaking
          encapsulation.
        </p>
        <p>
          Three different buttons are shown below with different custom styles.
        </p>
        ...
        >
          ...
          <ul style={{ margin: "10px 0", paddingLeft: "20px" }}>
            <li>
              <code>::part(ag-scrollto-button)</code> - The scroll-to-button
              wrapper element
            </li>
            <li>
              <code>::part(ag-button)</code> - The inner ag-button element
              (exported part)
            </li>
            <li>
              <code>::part(ag-button-content)</code> - The flex container for
              icon and label
            </li>
            <li>
              <code>::part(ag-icon-wrapper)</code> - The icon wrapper (control
              icon placement via flex order)
            </li>
            <li>
              <code>::part(ag-label)</code> - The label text element
            </li>
          </ul>
          <p
            style={{
          ...
          >
            The <code>ag-button</code> part is exported from the nested
            component, allowing full customization. Use{" "}
            <code>flex-direction</code> on <code>ag-button-content</code> to
            control icon/label layout (row, row-reverse, column,
            column-reverse).
          </p>
        </div>
        ...
        >
          ...
          <p>
            A beautiful gradient button with hover effects. Scroll down to see
            it appear!
          </p>
        ...
        >
          ...
          <p>
            The last two buttons demonstrate flexible icon placement using CSS
            parts.
          </p>
          <ul style={{ margin: "10px 0", paddingLeft: "20px" }}>
            <li>
              <strong>Icon on Right:</strong> Uses{" "}
              <code>flex-direction: row-reverse</code>
            </li>
            <li>
              <strong>Icon Above:</strong> Uses{" "}
  ...
  render: (args: ScrollToButtonProps) => (
    <>
      ...
      <div style={{ minHeight: "200vh", padding: "20px" }}>
        ...
        <p>
          This story demonstrates various icon and label layout options using
          CSS parts.
        </p>
        <p>
          All customization is done purely with CSS - no component modifications
          needed!
        </p>
        ...
        >
          ...
          <p style={{ margin: "10px 0" }}>
            The button content uses CSS Flexbox internally. By targeting the{" "}
            <code>::part(ag-button-content)</code> selector, you can control:
          </p>
          <ul
            style={{ margin: "10px 0", paddingLeft: "20px", lineHeight: 1.8 }}
          >
            <li>
              <code>flex-direction</code> - Controls icon/label order and
              orientation
            </li>
            <li>
              <code>gap</code> - Spacing between icon and label
            </li>
            <li>
              <code>padding</code> - Button internal spacing
            </li>
          </ul>
          <p style={{ marginTop: "10px" }}>
            You can also target <code>::part(ag-icon-wrapper)</code> and{" "}
            <code>::part(ag-label)</code> independently for granular styling.
          </p>
        </div>
        ...
        >
          ...
          >
            <li>
              <strong>Default (Icon Left):</strong> Standard left-aligned icon
              with label
            </li>
            <li>
              <strong>Icon Right:</strong> Label on left, icon on right
            </li>
            <li>
              <strong>Icon Above:</strong> Icon stacked above label (vertical)
            </li>
            <li>
              <strong>Icon Below:</strong> Label above icon (inverted vertical)
            </li>
            <li>
              <strong>Icon in Circle Above:</strong> Circular icon container
              above label
            </li>
          </ol>
          {Array.from({ length: 20 }).map((_, i) => (
            <p key={i} style={{ marginBottom: "40px", lineHeight: 1.6 }}>
              Paragraph {i + 1}: Scroll down to see all the button positioning
              variations.
            </p>
          ))}
        </section>
...
export const ButtonShapes: Story = {
  render: (args: ScrollToButtonProps) => (
    <div style={{ minHeight: "200vh", padding: "20px" }}>
      <h1>Button Shapes</h1>
      <p>
        This story showcases different button shapes available via the{" "}
        <code>shape</code> prop.
      </p>
      <p>
        Scroll down to try out the various shapes: square, rounded, capsule,
        circle, and rounded-square on the lower right.
      </p>

      <ReactScrollToButton shape="square" style={{ bottom: "170px" }} />
