...
const meta: Meta<typeof ReactBackToTop> = {
  title: 'AgnosticUI React/BackToTop',
  component: ReactBackToTop,
  tags: ['autodocs'],
  argTypes: {
    visible: {
      control: 'boolean',
  ...
  render: (args) => (
    <div style={{ minHeight: '200vh', padding: '20px' }}>
      ...
      {Array.from({ length: 20 }).map((_, i) => (
        <p key={i} style={{ marginBottom: '50px' }}>Scroll more...</p>
      ))}
      <ReactBackToTop {...args} />
    </div>
  ),
  ...
  render: (args) => (
    <>
      ...
      <div style={{ minHeight: '200vh', padding: '20px' }}>
        ...
        <ReactBackToTop className="custom-button" {...args} />
      </div>
    </>
  ),
  args: {
