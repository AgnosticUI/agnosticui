...
describe('AgInput - Basic Functionality', () => {
  ...
  describe('v1 Parity - Styling Variants', () => {
    ...
    describe('Boolean Styling Properties', () => {
      it('applies isRounded variant', async () => {
        const element = new AgInput();
        element.label = 'Rounded Input';
        element.isRounded = true;
        document.body.appendChild(element);
        await element.updateComplete;

        expect(element.hasAttribute('is-rounded')).toBe(true);
        const wrapper = element.shadowRoot?.querySelector('.ag-input');
        expect(wrapper?.classList.contains('ag-input--rounded')).toBe(true);
      });

      it('applies isUnderlined variant', async () => {
        const element = new AgInput();
        ...
        expect(element.hasAttribute('is-underlined')).toBe(true);
        const wrapper = element.shadowRoot?.querySelector('.ag-input');
        expect(wrapper?.classList.contains('ag-input--underlined')).toBe(true);
      });

      it('applies isUnderlinedWithBackground variant', async () => {
        const element = new AgInput();
        element.label = 'Underlined with Background';
        element.isUnderlinedWithBackground = true;
        document.body.appendChild(element);
        await element.updateComplete;

        expect(element.hasAttribute('is-underlined-with-background')).toBe(true);
        const wrapper = element.shadowRoot?.querySelector('.ag-input');
        expect(wrapper?.classList.contains('ag-input--underlined-with-background')).toBe(true);
      });

      it('applies isInline variant', async () => {
        const element = new AgInput();
        ...
        expect(element.hasAttribute('is-inline')).toBe(true);
        // Note: CSS :host([is-inline]) rule will set display: inline-block
