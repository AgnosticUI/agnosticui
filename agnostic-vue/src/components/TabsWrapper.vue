<template>
  ...
  >
    <button
      ...
      :disabled="isTabDisabled(title)"
      :tabindex="selectedTitle === title ? '0' : '-1'"
      :aria-selected="selectedTitle === title"
      :class="tabButtonClasses(title)"
    >
      <slot :name="tabButtonSlotName(title)">
        {{ title }}
      </slot>
    </button>
    <slot />
...
export default {
  ...
  methods: {
    tabButtonSlotName(tabTitle) {
      const stripLoweredTabTitle = tabTitle.replace(" ", "").toLowerCase();
      console.log("tabButtonSlotName: ", `tab-${stripLoweredTabTitle}`);
      return `tab-${stripLoweredTabTitle}`;
    },
    ...
    focusTab(index, direction) {
      ...
      if (i < 0) {
        i = this.tabTitles.length - 1;
      } else if (i >= this.tabTitles.length) {
        // We've went beyond "last" so circle around to first
        i = 0;
      }

      console.log("tabButtonRefs: ", this.tabButtonRefs);
      const nextTabRef = this.tabButtonRefs[i];
      console.log("nextTabRef", nextTabRef);
      if (nextTabRef) {
        ...
        if (nextTabRef.disabled && direction) {
      ...
      /*
      // const nextTabRef = this.$refs[this.tabTitles[i]];
      console.log("nextTab", nextTab);
      if (nextTab) {
        // Edge case: We hit a tab button that's been disabled. If so, we recurse, but
        // only if we've been supplied a `direction`. Otherwise, nothing left to do.
    ...
    onKeyDown(ev, index) {
      ...
      switch (ev.key) {
        ...
        case "End":
        case "ArrowEnd":
          this.focusTab(this.tabTitles.length - 1);
          break;
        case "Enter":
        case "Space":
          this.focusTab(index);
          this.selectTab(this.tabTitles[index]);
          break;
        default:
          return;
      }
